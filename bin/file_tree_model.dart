import 'dart:io';

import 'params.dart';
import 'widget_info.dart';



class FileTreeBranch{
  
    Map<String, dynamic> branchFiles={};
    Map<String, FileTreeBranch> branchDirs ={};

    FileTreeBranch();

    addBranchDir(String dirName)=>
      branchDirs[dirName]= FileTreeBranch();

    addBranchFile(String fileName, dynamic widgetInfo)=>
       branchFiles[fileName]=widgetInfo;
}



class FileTreeRoot {
  String dirPath;
  String importPath;
  Map<String, dynamic> rootFiles={};
  Map<String, FileTreeBranch> branchDirs ={};
  // Library

  FileTreeRoot({this.dirPath, this.importPath});

  addRootFile(String fileName, dynamic widgetInfo)=>
    rootFiles[fileName]=widgetInfo;
  addBranchDirs(List<String> dirs){
    dirs.forEach((d)=>branchDirs[d]= FileTreeBranch());
  }
  addBranchDir(String dirName)=>
    branchDirs[dirName]= FileTreeBranch( );

  String getPackagePath({String fileName, String dirName=""}){
    if(dirName!="" && branchDirs.containsKey(dirName)){
      if(branchDirs[dirName].branchFiles.containsKey(fileName)){
        return "$importPath/$dirName/$fileName";
      }
    }
    if(rootFiles.containsKey(fileName))return "$importPath/$fileName";
    String out="";
    branchDirs.forEach((branchName, branch){
      if(branch.branchFiles.containsKey(fileName)){
        out="$importPath/$dirName/$fileName";
        return;
      } 
    });
    return out;
  }
  List<Param> getParams({String fileName, String dirName=""}){
    if(dirName!="" && branchDirs.containsKey(dirName)){
      if(branchDirs[dirName].branchFiles.containsKey(fileName)){
        return branchDirs[dirName].branchFiles[fileName].params;
      }
    }
    if(rootFiles.containsKey(fileName))return rootFiles[fileName].params;

    List<Param> p=[];
    branchDirs.forEach((branchName, branch){
      if(branch.branchFiles.containsKey(fileName)){
        p=branch.branchFiles[fileName].params;
        return;
      } 
    });
    return p;
  }


  create(){
    rootFiles.forEach((filename, fileInfo){
      String imports= addImports(fileInfo.packages);
      createFile(fileInfo.build(imports), "$dirPath/$filename.dart");
    });
    

    branchDirs.forEach((branchName, branchFileTree)=>createBranch(branchName, branchFileTree));
  }

  createBranch(branchName, branchFileTree){
    createDirectory(branchName);
    // branchFileTree.branchDirs?.forEach((childBranchName, childBranchFileTree){
    //    createBranch("$branchName/$childBranchName", childBranchFileTree);
    // });

    branchFileTree.branchFiles?.forEach((branchFileName, branchFileInfo){
      String imports= addImports(branchFileInfo.packages);
      createFile(branchFileInfo.build(imports), "$dirPath/$branchName/$branchFileName.dart");
    });
    
  }
  createDirectory(String path){
    if(!Directory("$dirPath/$path").existsSync()){
      Directory("$dirPath/$path").createSync(recursive: true);
    }  
  }
  createFile(String code, String path){
    File(path)..createSync(recursive: true)..writeAsStringSync(code);
  }


  String addImports(List<PacMan> filePackages){
    String out="";
    filePackages.forEach((p){
      String s = p.dirName;
      if (p.local){
        s= getPackagePath(fileName:p.fileName, dirName:p.dirName);
      }
      if (p.asIs)out+="import '$s';\n";
      else out+="import '$s.dart';\n";
    });
    return out;
  }
 

}



  //String dirPath;
    //String importPath;

     // getPackagePath: getPackagePath
          // dirPath: "$dirPath/$dirName",
          // importPath: "$importPath/$dirName"
   // rootFiles[fileName].getPackagePath = getPackagePath;
   //getPackagePath: getPackagePath
          // dirPath: "$dirPath/$dirName",
          // importPath: "$importPath/$dirName"


  // branchDirs.putIfAbsent(
    //   dirName, ()=>FileTreeBranch(
    //     getPackagePath: getPackagePath
    //     // dirPath: "$dirPath/$dirName",
    //     // importPath: "$importPath/$dirName"
    // ));




    // addModel(String name, Map data)
    // addSource()
    // addScreen(String name, Map data, )
    // addComponent()
    // addController()


// class FileTreeModel {
//   String dirPath;
//   String importPath;
//   String baseName;
//   String className;

//   FileTreeModel({this.className, this.baseName, this.dirPath, this.importPath});
//   FileTreeModel branch(String _baseName){
//     return FileTreeModel(
//         baseName: _baseName,
//         importPath: importPath,
//         dirPath: dirPath
//         );
//   }
 
//   FileTreeModel branchModel() {
//     //baseName = _baseName; // setting roots base name to model for each model
//     return FileTreeModel(
//         className: capWord(baseName)+"Model",
//         baseName: baseName,
//         importPath: "$importPath/models/$baseName.dart",
//         dirPath: "$dirPath/models/$baseName.dart");
//   }

//   FileTreeModel branchScreen(String screentype) {
//     return FileTreeModel(
//         className: capWord(baseName)+capWord(screentype),
//         baseName: baseName,
//         importPath: "$importPath/screens/${baseName}_$screentype.dart",
//         dirPath: "$dirPath/screens/${baseName}_$screentype.dart");
//   }

//   FileTreeModel branchComponent(String componenttype) {
//     return FileTreeModel(
//         className: capWord(baseName)+capWord(componenttype),
//         baseName: baseName,
//         importPath: "$importPath/components/${baseName}_$componenttype.dart",
//         dirPath: "$dirPath/components/${baseName}_$componenttype.dart");
//   }

//   FileTreeModel branchSource(String _sourceName) {
//     return FileTreeModel(
//         className: capWord(_sourceName),
//         importPath: "$importPath/sources/$_sourceName.dart",
//         dirPath: "$dirPath/sources/$_sourceName.dart");
//   }

//   FileTreeModel branchFile(String _fileName){
//     return FileTreeModel(
//         className: capWord(_fileName),
//         importPath: "$importPath/$_fileName.dart",
//         dirPath: "$dirPath/$_fileName.dart"
//         );
//   }

//   createFolders(List<String> newDirs){
//     newDirs.forEach((dirName)=>createDirectory("$dirPath/$dirName"));
    
//   }
//   createDirectory(String dirPath){
//     if(!Directory(dirPath).existsSync()){
//       Directory(dirPath).createSync(recursive: true);
//     }  
//   }
//   createFile(String code){
//     if(dirPath.contains(".dart"))
//       File(dirPath)..createSync(recursive: true)..writeAsStringSync(code);
//   }

// }

// String capWord(String word){
//   if(!word.contains("_"))return word[0].toUpperCase()+word.substring(1);
//   String out = "";
//   word.split("_").forEach((w)=>out+=capWord(w));
//   return out;
// }


// import 'dart:io';

// createDirectory(String dirPath){
//   if(!Directory(dirPath).existsSync()){
//     Directory(dirPath).createSync(recursive: true);
//   }  
// }


// createFile(String filePath, String code){
//     File(filePath)..createSync(recursive: true)..writeAsStringSync(code);
// }